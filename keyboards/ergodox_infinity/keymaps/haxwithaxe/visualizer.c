...
static void get_visualizer_layer_and_color(visualizer_state_t* state) {
    ...
    if (state->status.layer & 0x400) {
        ...
        state->target_lcd_color = LCD_COLOR(L6_COLOR, saturation, 0xFF);
        state->layer_text = L6_NAME;
    } else if (state->status.layer & 0x20) {
        state->target_lcd_color = LCD_COLOR(L5_COLOR, saturation, 0xFF);
        state->layer_text = L5_NAME;
    } else if (state->status.layer & 0x10) {
        state->target_lcd_color = LCD_COLOR(L4_COLOR, saturation, 0xFF);
        state->layer_text = L4_NAME;
    } else if (state->status.layer & 0x8) {
        state->target_lcd_color = LCD_COLOR(L3_COLOR, saturation, 0xFF);
        state->layer_text = L3_NAME;
    } else if (state->status.layer & 0x4) {
        state->target_lcd_color = LCD_COLOR(L2_COLOR, saturation, 0xFF);
        state->layer_text = L2_NAME;
        stop_keyframe_animation(&led_test_animation);
        start_keyframe_animation(&sweep_on_sweep_off_left_and_right);
    } else if (state->status.layer & 0x2) {
        state->target_lcd_color = LCD_COLOR(L1_COLOR, saturation, 0xFF);
        state->layer_text = L1_NAME;
    } else {
        state->target_lcd_color = LCD_COLOR(L0_COLOR, saturation, 0xFF);
        state->layer_text = L0_NAME;
        stop_keyframe_animation(&led_test_animation);
